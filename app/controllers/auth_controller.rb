class AuthController < ApplicationController
    ...
    def company_create
      ...
      if(company && company.authenticate(params[:password]))
        ...
        new_hash = {id: company.id, email: company.email, name: company.name, courses: company.courses, purchases: company.purchases}
        new_hash["token"] = token
        company = new_hash
        render :json => {company: company.as_json(except: [:created_at, :updated_at]), token: token}
